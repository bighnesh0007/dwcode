            ...
            fc.assert(
                fc.property(
                    ...
                    (
                        ...
                        // The property that must hold now the fix is in place: computing
                        // stats over the FULL mixed collection, filtered by userId, must
                        // give exactly the same answer as computing over userA's records
                        // alone. This is the regression guard for the userId filter in
                        // app/page.tsx and the /api/{submissions,bookmarks} routes.
                        const actualForA = computeStats(
                            allSubmissions,
                            allBookmarks,
                            userAId
                        );

                        expect(actualForA.solved).toEqual(expectedForA.solved);
                        expect(actualForA.attempted).toEqual(expectedForA.attempted);
                        expect(actualForA.bookmarked).toEqual(expectedForA.bookmarked);
                    }
                ),
                {
                    numRuns: 200,
...
/**
 * Historical record of the original defect.
 *
 * `computeStatsBuggy` reproduces the pre-fix logic (no `userId` filter). This test
 * pins a concrete counterexample so the reason the filter exists stays documented and
 * cannot be "simplified away" by someone reading the fixed code in isolation.
 */
describe("historical bug: unfiltered stats leak other users' data", () => {
    const subs: SubmissionDoc[] = [
        { userId: "user_a", problemSlug: "a-1", status: "Accepted" },
        { userId: "user_b", problemSlug: "b-1", status: "Accepted" },
        { userId: "user_b", problemSlug: "b-2", status: "Accepted" },
    ];
    const bookmarks: BookmarkDoc[] = [
        { userId: "user_a", problemId: "p1" },
        { userId: "user_b", problemId: "p2" },
    ];

    it("the old implementation inflated userA's counts with userB's records", () => {
        const buggy = computeStatsBuggy(subs, bookmarks, "user_a");
        // Platform-wide totals rather than userA's own.
        expect(buggy.solved).toBe(3);
        expect(buggy.bookmarked).toBe(2);
    });

    it("the current implementation reports only userA's own records", () => {
        const correct = computeStats(subs, bookmarks, "user_a");
        expect(correct.solved).toBe(1);
        expect(correct.bookmarked).toBe(1);
    });

    it("guests see zeroes rather than platform totals", () => {
        expect(computeStats(subs, bookmarks, null)).toEqual({
            solved: 0,
            attempted: 0,
            bookmarked: 0,
        });

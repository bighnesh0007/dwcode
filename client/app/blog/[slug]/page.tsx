    ...
    return (
        <div className="container max-w-screen-md mx-auto py-10 px-4 space-y-6">
            ...
            {/*
              ...
              NOTE: the `prose prose-sm dark:prose-invert` classes that used to be here
              were dead — @tailwindcss/typography is not installed, so they styled
              nothing (and `dark:prose-invert` never inverted anything). All real
              styling comes from lib/markdown.ts, which emits theme-token classes
              (bg-muted, text-primary, text-muted-foreground) that adapt to dark mode.

              The rules below cover the elements that renderMarkdown does NOT class
              directly, so long-form posts get sane rhythm in both themes.
            */}
            <article
                className="max-w-none text-[0.95rem] leading-7 text-foreground/90
                           [&_a]:break-words
                           ...
                           [&_li]:my-1
                           [&_pre]:border [&_pre]:border-border/60

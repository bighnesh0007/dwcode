            ...
            {loading ? (
                ...
                <div className="grid gap-5">
                    {posts.map(post => {
                        ...
                        return (
                            /* In dark mode a shadow is invisible, so lift the card with a
                               brighter border and a subtle surface change instead. */
                            <Card
                                key={post._id}
                                className="group transition-all hover:shadow-md hover:border-primary/30 dark:hover:bg-card/80 dark:hover:shadow-none"
                            >
                                ...
                                <CardContent className="pt-0 space-y-3">
                                    ...
                                    {linkedInUrl ? (
                                        <a
                                            ...
                                            /* LinkedIn brand blue (#0A66C2) is too dark to read on a dark
                                               card, so dark mode lightens both the text and the wash. */
                                            className="group/li flex items-center gap-3 rounded-lg border border-[#0A66C2]/30 bg-[#0A66C2]/5 px-4 py-3 transition-colors hover:bg-[#0A66C2]/10 dark:border-[#70B5F9]/25 dark:bg-[#70B5F9]/10 dark:hover:bg-[#70B5F9]/20"
                                        >
                                            <LinkedInIcon className="w-5 h-5 shrink-0 text-[#0A66C2] dark:text-[#70B5F9]" />
                                            <div className="flex-1 min-w-0">
                                                <p className="text-sm font-medium text-[#0A66C2] dark:text-[#70B5F9]">LinkedIn Post</p>
                                                {preview && (
                                                    <p className="text-xs text-muted-foreground line-clamp-2 mt-0.5">{preview}</p>
                                                )}
                                            </div>
                                            <ExternalLink className="w-3.5 h-3.5 shrink-0 text-[#0A66C2] opacity-60 group-hover/li:opacity-100 dark:text-[#70B5F9]" />
                                        </a>
                                    ) : (
                                        /* Plain text preview */
                                        preview && (
